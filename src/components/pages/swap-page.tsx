...
import { ArrowLeft, ArrowLeftRight, AlertTriangle, CheckCircle, Loader2 } from "lucide-react";
...
export function SwapPage({ onBack }: { onBack: () => void }) {
  ...
  const [swapStep, setSwapStep] = useState<"input" | "confirm" | "swapping" | "done">("input");
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 px-5 py-6">
        ...
        {fromAmount && parseFloat(fromAmount) > 1 && (
          <div className="mt-4 bg-amber-50 border border-amber-100 rounded-2xl p-3 flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" />
            <p className="text-xs text-amber-700">Large swap may have price impact. Consider splitting into smaller amounts.</p>
          </div>
        )}
        <div className="mt-8">
          <button onClick={() => fromAmount && setSwapStep("confirm")} disabled={!fromAmount}
            className="w-full identity-gradient disabled:opacity-40 text-white py-4 rounded-2xl font-semibold text-lg shadow-lg active:scale-[0.98] transition-transform">
            Review Swap
          </button>
        </div>
        ...
        {swapStep === "confirm" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-sm mx-4 bg-white rounded-3xl shadow-2xl p-6 animate-fade-in-up">
              <h3 className="text-lg font-bold text-center mb-4">Confirm Swap</h3>
              <div className="bg-gray-50 rounded-2xl p-4 space-y-3 mb-4">
                ...
                <div className="flex justify-center"><ArrowLeftRight className="w-4 h-4 text-gray-300" /></div>
                <div className="flex justify-between text-sm">
              ...
              <div className="flex gap-3">
                <button onClick={() => setSwapStep("input")} className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-2xl font-semibold active:scale-[0.98]">Cancel</button>
                <button onClick={() => { setSwapStep("swapping"); setTimeout(() => setSwapStep("done"), 2000); }} className="flex-1 identity-gradient text-white py-3 rounded-2xl font-semibold shadow-lg active:scale-[0.98]">Confirm Swap</button>
              </div>
            </div>
          </div>
        )}
        ...
        {swapStep === "swapping" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl p-8 text-center shadow-2xl animate-fade-in-up">
              <Loader2 className="w-12 h-12 text-blue-500 animate-spin mx-auto mb-4" />
              <p className="font-bold text-lg">Swapping...</p>
              <p className="text-sm text-gray-400 mt-1">Finding best route</p>
            </div>
          </div>
        )}
        ...
        {swapStep === "done" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl p-8 text-center shadow-2xl animate-fade-in-up">
              ...
              <p className="font-bold text-lg">Swap Complete!</p>
              <p className="text-sm text-gray-400 mt-1">{fromAmount} {fromToken} → {toAmount} {toToken}</p>
              <button onClick={() => { setSwapStep("input"); setFromAmount(""); }} className="mt-4 px-6 py-2.5 identity-gradient text-white rounded-2xl font-semibold shadow-lg active:scale-[0.98]">Done</button>
            </div>
          </div>
        )}
      </div>
